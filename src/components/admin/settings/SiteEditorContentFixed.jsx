...
const SiteEditorContent = () => {
  ...
  const renderContactEditor = () => {
    ...
    const updateContactField = (section, field, value) => {
      const updatedContact = { ...contact };
      if (section) {
        updatedContact[section] = { ...updatedContact[section], [field]: value };
      } else {
        updatedContact[field] = value;
      }
      updateSiteData('contact', updatedContact);
    };

    const addWhyChooseItem = () => {
      const whyChoose = contact.whyChoose || { title: '', items: [] };
      const newItem = { icon: '⭐', title: 'Novo Item', description: 'Descrição...' };
      const updatedItems = [...(whyChoose.items || []), newItem];
      updateContactField('whyChoose', 'items', updatedItems);
    };

    const removeWhyChooseItem = (index) => {
      const whyChoose = contact.whyChoose || { items: [] };
      const updatedItems = whyChoose.items.filter((_, i) => i !== index);
      updateContactField('whyChoose', 'items', updatedItems);
    };

    const updateWhyChooseItem = (index, field, value) => {
      const whyChoose = contact.whyChoose || { items: [] };
      const updatedItems = [...whyChoose.items];
    ...
    const addService = () => {
      ...
      const newService = { icon: '💼', title: 'Novo Serviço', description: 'Descrição do serviço...' };
      const updatedServices = [...services, newService];
      updateSiteData('contact', { ...contact, services: updatedServices });
    };
